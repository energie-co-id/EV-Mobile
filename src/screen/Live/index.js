  ...
  return (
    ...
    <View style={[tw`flex flex-row bg-white`]}>
     <TouchableOpacity onPress={() => props.navigation.navigate("Charge")}><Image style={[tw`h-5 w-5 mt-7.8 ml-5 mb-5`]} source={require('../../assets/back.png')} /></TouchableOpacity>
      <Text style={[tw`text-3xl text-center text-gray-700 ml-21 mt-7 mb-5 font-bold font-mono`]}>On Charging</Text>
     ...
     <View style={[tw`bg-white w-40 mt-3`]}>
    ...
    <View style={[tw`bg-white w-38 mt-3 ml-3.5`]}>
     ...
     <TouchableOpacity onPress={handleCondition} style={[tw`rounded-full flex flex-col ${condition === 'on' ? 'bg-green-100 border-4 border-green-500' : 'bg-gray-100'} h-30 w-30 justify-center items-center ml-1 mt-5 mb-49`]}>
     <Icon style={[{ color: condition=== 'on' ? '#7fff94' : '#FF5C5C'},tw`mt-1 justify-center items-center`]} name='power-settings-new' size={95} />
        </TouchableOpacity>
